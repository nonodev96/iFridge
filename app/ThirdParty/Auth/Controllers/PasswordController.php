...
class PasswordController extends Controller
{
    ...
    public function attemptForgotPassword(): \CodeIgniter\HTTP\RedirectResponse
    {
        // validate request
        ...
        helper('auth');
        $status = send_password_reset_email($this->request->getPost('email'), $updatedUser['reset_hash']);

        if (!$status) {
            return redirect()->back()->with('success', lang('Auth.forgottenPasswordEmail'));
        } else {
            return redirect()->back()->with('success', lang('Auth.forgottenPasswordEmailFail'));
        }
    ...
    public function resetPassword()
    {
        ...
        $users = new UserModel();
        $user  = $users->where('reset_hash', $this->request->getGet('token'))->where('reset_expires >', time())->first();
    ...
    public function attemptResetPassword(): \CodeIgniter\HTTP\RedirectResponse
    {
        // validate request
        $rules = [
                  'token'            => 'required',
        ...
        $users = new UserModel();
        $user  = $users->where('reset_hash', $this->request->getPost('token'))->where('reset_expires >', time())->first();
